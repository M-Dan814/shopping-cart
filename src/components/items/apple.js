...
const Apple = () => {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("0");
    if (stored) {
      const obj = JSON.parse(stored);
      setLeft(5 - obj.quantity);
  ...
  const addToCart = () => {
    const stored = sessionStorage.getItem("0");
    if (!stored) {
      const obj = { name: "Apple", price: 0.2, quantity: 1 };
    ...
    <div className="min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-10 py-10 px-4">
        <div className="text-center">
          <h1 className="text-5xl font-cursive text-green-700 mb-4">Apples</h1>
          <img
            src={Image}
            alt="Apples"
