...
const Image = require("../images/tomatoes.jpg");
...
const Tomatoes = () => {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("10");
    if (stored) {
  ...
  const addToCart = () => {
    const stored = sessionStorage.getItem("10");
    if (!stored) {
      const obj = { name: "Tomatoes", price: 0.25, quantity: 1 };
      sessionStorage.setItem("10", JSON.stringify(obj));
      setAdded(1);
      setLeft(11);
    } else {
      const obj = JSON.parse(stored);
    ...
    <div className="min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-10 py-10 px-4">
        <div className="text-center">
          <h1 className="text-5xl font-cursive text-red-700 mb-4">Tomatoes</h1>
          <img
            src={Image}
            alt="Tomatoes"
        ...
        <div className="flex flex-col items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <span className="text-3xl font-semibold text-red-700">{added}</span>
            <button
              onClick={addToCart}
              disabled={left === 0}
              className={`text-2xl font-bold px-5 py-3 rounded-lg transition-all ${
