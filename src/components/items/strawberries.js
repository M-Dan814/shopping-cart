...
const Strawberries = () => {
  ...
  useEffect(() => {
    if (sessionStorage.getItem("9") != null) {
      const stringed = sessionStorage.getItem("9");
  ...
  const addToCart = () => {
    if (sessionStorage.getItem("9") === null) {
      ...
      sessionStorage.setItem("9", stringed);
      setAdded(added + 1);
      setLeft(left - 1);
    } else {
      const string = sessionStorage.getItem("9");
  ...
  return (
    <>
      ...
      <div className="item-cont">
        <div className="item">
          <h1>Strawberries</h1>
          <img src={Image} width="800" alt="strawberries" height={600} />
        </div>
