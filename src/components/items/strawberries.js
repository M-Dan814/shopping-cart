...
const Strawberries = () => {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem("9");
    if (stored) {
  ...
  const addToCart = () => {
    const stored = sessionStorage.getItem("9");
    if (!stored) {
      const obj = { name: "Strawberries", price: 0.35, quantity: 1 };
      sessionStorage.setItem("9", JSON.stringify(obj));
      setAdded(1);
      setLeft(11);
    } else {
      const obj = JSON.parse(stored);
    ...
    <div className="min-h-screen">
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-10 py-10 px-4">
        <div className="text-center">
          <h1 className="text-5xl font-cursive text-red-600 mb-4">
            Strawberries
          </h1>
          <img
            src={Image}
        ...
        <div className="flex flex-col items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <span className="text-3xl font-semibold text-red-600">{added}</span>
            <button
              onClick={addToCart}
              disabled={left === 0}
              className={`text-2xl font-bold px-5 py-3 rounded-lg transition-all ${
